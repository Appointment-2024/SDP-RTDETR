...
class UniRepLKNetBlock(nn.Module):
    ...
    def reparameterize(self):
        ...
        if hasattr(self.norm, 'running_var'):
            # 兼容两种情况：
            # - 大核块部署后，真正的卷积在 self.dwconv.lk_origin
            # - 普通卷积时，卷积就是 self.dwconv
            ...
            if conv is not None:
                ...
                # 处理 conv.bias is None 的情况
                if conv.bias is None:
                    conv_bias = torch.zeros_like(self.norm.running_mean)
                else:
                    conv_bias = conv.bias.data

                # fuse weight
                conv.weight.data.mul_((self.norm.weight / std).view(-1, 1, 1, 1))

                # fuse bias
                fused_bias = self.norm.bias + (conv_bias - self.norm.running_mean) * self.norm.weight / std

                if conv.bias is None:
                    conv.bias = nn.Parameter(fused_bias.clone())
                else:
